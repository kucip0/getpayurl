...
# 导入登录和商品修改模块
from login import login_to_houfaka, get_goods_edit_data, modify_goods_price
...
class AutoOrderApp:
    ...
    def __init__(self, root):
        ...
        # 平台配置（默认猴发卡）
        self.platform_host = "https://www.houfaka.com"
        ...
        # 持久化 sc447eeeb Cookie（首次获取后复用）
        self.saved_sc447eeeb = None
        ...
        # Cookie 持久化文件路径
        self.cookie_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "merchant_cookies.pkl")
        
        # 商品链接持久化文件路径
        self.url_config_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "url_config.pkl")
        
        # 登录凭证持久化文件路径
        self.credentials_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "login_credentials.pkl")
    ...
    def _on_get_price(self):
        ...
        def get_price_thread():
            try:
                ...
                resp = self.merchant_session.get(f"{base_url}{path}", timeout=15)
    ...
    def _on_login(self):
        ...
        def do_login():
            ...
            def login_thread():
                try:
                    # 使用当前平台的 host 进行登录
                    session, cookies = login_to_houfaka(username, password, base_url=self.platform_host)
                    ...
                    # 从登录 Cookie 中提取 sc447eeeb
                    for cookie in session.cookies:
                        if 'sc447eeeb' in cookie.name.lower():
                            self.saved_sc447eeeb = cookie.value
                            self._log(f"成功: 从登录响应中提取 sc447eeeb Cookie: {self.saved_sc447eeeb[:10]}...")
                            break
    ...
    def _worker(self, url):
        ...
        try:
            ...
            alipay_params = self._step3_get_alipay_form(cookie, orderid)
    ...
    def _step1_get_cookie_and_params(self, url):
        ...
        # 优先使用已保存的 Cookie
        if self.saved_sc447eeeb:
            cookie_value = self.saved_sc447eeeb
            self._log(f"复用已保存的 sc447eeeb Cookie: {cookie_value[:10]}...")
        else:
            # 首次获取，从响应中提取
            for cookie_name, cookie_val in resp.cookies.items():
                if "sc447eeeb" in cookie_name.lower():
            ...
            if not cookie_value:
                ...
                match = re.search(r'sc447eeeb=([^;]+)', set_cookies, re.IGNORECASE)
                if match:
                    cookie_value = match.group(1)
                else:
                    raise Exception("步骤1: 未找到 sc447eeeb Cookie")

            # 保存 Cookie 供后续复用
            self.saved_sc447eeeb = cookie_value
            self._log(f"首次获取并保存 sc447eeeb Cookie: {cookie_value[:10]}...")
    ...
    def _step2_submit_order(self, cookie, params):
        ...
        # 更新 Cookie
        self.session.cookies.set("sc447eeeb", cookie)
    ...
    def _step3_get_alipay_form(self, cookie, orderid):
        ...
        resp = self.session.get(
            f"{base_url}/index/pay/payment",
            params={"trade_no": orderid, "agree": "on"},
            headers=headers,
            cookies={"sc447eeeb": cookie},
            timeout=15
        )
    ...
    def _step4_request_alipay_gateway(self, alipay_params):
        ...
        ordered_params["notify_url"] = f"{self.platform_host}/pay/alipay_wap/notify.html"
        ...
        self._log(f"步骤4请求体: {encoded_data}")
    ...
    def _load_merchant_cookies(self):
        ...
        try:
            ...
            for cookie in cookies_list:
                self.merchant_session.cookies.set(cookie['name'], cookie['value'], domain=cookie.get('domain', '.houfaka.com'))
                # 提取 sc447eeeb Cookie 供支付二维码流程复用
                if 'sc447eeeb' in cookie['name'].lower():
                    self.saved_sc447eeeb = cookie['value']
            ...
            if self._verify_merchant_cookie():
                ...
                if self.saved_sc447eeeb:
                    self._log(f"成功: 从文件加载 sc447eeeb Cookie: {self.saved_sc447eeeb[:10]}...")
            else:
