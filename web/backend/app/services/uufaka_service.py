...
class UufakaService(BaseService):
    """悠悠发卡服务"""

    PLATFORM_CODE = "uufaka"
    BASE_URL = "https://www.uufaka.com"
    ...
    def submit_order(self, product_url: str, new_price: float) -> dict:
        ...
        try:
            ...
            for channel in channels:
                # 悠悠发卡使用name字段区分支付方式（code都是Epay）
                name = channel.get("name", "") + channel.get("show_name", "")
                if "支付宝" in name:
                    alipay_channel_id = channel.get("id")
                    break

            if not alipay_channel_id:
                raise Exception("未找到支付宝支付渠道")
            ...
            # 步骤5: 访问支付页面，获取302重定向到Epay页面
            payment_url = f"{self.BASE_URL}/shopApi/Pay/payment?trade_no={trade_no}"
            
            self.log("=" * 80)
            self.log("【悠悠发卡-步骤5】访问支付页面获取重定向 - 请求信息")
            ...
            self.log("=" * 80)
            self.log("【悠悠发卡-步骤5】访问支付页面获取重定向 - 响应信息")
            self.log(f"响应状态码: {resp.status_code}")
            self.log(f"响应头:")
            for k, v in resp.headers.items():
                self.log(f"  {k}: {v}")
            ...
            if resp.status_code not in (302, 301):
                raise Exception(f"步骤5: 预期302重定向，实际HTTP {resp.status_code}")
            
            location = resp.headers.get("Location")
            if not location:
                raise Exception("步骤5: 响应头中无Location字段")
            
            self.log(f"步骤5: 302重定向到: {location}")
            
            # 步骤6: 跟随重定向访问Epay页面，获取二维码URL
            if location.startswith("/"):
                epay_url = f"{self.BASE_URL}{location}"
            else:
                epay_url = location
            
            self.log("=" * 80)
            self.log("【悠悠发卡-步骤6】访问Epay页面获取二维码URL - 请求信息")
            self.log(f"请求URL: {epay_url}")
            self.log(f"请求方法: GET")
            self.log("=" * 80)
            
            resp = self.session.get(epay_url, headers={
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/147.0.0.0 Safari/537.36",
                "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
                "Referer": product_url,
                "sec-ch-ua": '"Google Chrome";v="147", "Not.A/Brand";v="8", "Chromium";v="147"',
            ...
            self.log("=" * 80)
            self.log("【悠悠发卡-步骤6】访问Epay页面获取二维码URL - 响应信息")
            self.log(f"响应状态码: {resp.status_code}")
            self.log(f"完整响应体:")
            self.log(resp.text)
            self.log("=" * 80)
            ...
            code_img = soup.find("img", class_="code")
            if not code_img:
                raise Exception(f"未找到二维码图片，响应体: {resp.text}")
            
            img_src = code_img.get("src", "")
            if not img_src:
                raise Exception("二维码图片src为空")
            
            self.log(f"步骤6: 二维码img src: {img_src}")
            
            # 从src中提取URL参数（/payApi/common/generateQrcode.html?str=xxx）
            from urllib.parse import unquote
            if "?str=" in img_src:
                qr_url_encoded = img_src.split("?str=")[1]
                # URL解码（可能需要多次解码）
                qr_url = unquote(unquote(qr_url_encoded))
            else:
                # 如果不是完整的URL，拼接BASE_URL
                if img_src.startswith("/"):
                    qr_url = f"{self.BASE_URL}{img_src}"
                else:
                    qr_url = img_src
            
            self.log(f"步骤6: 解码后的二维码URL: {qr_url}")

            # 步骤7: 生成二维码
            qr_base64 = self._generate_qrcode(qr_url)
    ...
    def login(self, username: str, password: str, verify_code: str = "", csrf_token: str = "") -> dict:
        ...
        try:
            ...
            self.session.cookies.set("merchant-token", token, domain="www.uufaka.com")
