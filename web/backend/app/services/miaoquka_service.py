...
class MiaoqukaService(BaseService):
    """秒取卡服务"""

    PLATFORM_CODE = "miaoquka"
    BASE_URL = "https://dev.miaoquka.cn"
    TRACKING_COOKIE = "sb8daea6e"

    def login(self, username: str, password: str, verify_code: str = "", csrf_token: str = "") -> dict:
        """登录秒取卡店铺"""
        try:
            # 使用临时session（避免旧Cookie影响）
            temp_session = requests.Session()
            
            # 1. 访问主页获取初始Cookie
            resp = temp_session.get(f"{self.BASE_URL}/")
            resp.raise_for_status()

            # 2. 访问登录页提取token
            resp = temp_session.get(f"{self.BASE_URL}/login")
            resp.raise_for_status()
            ...
            # 调试日志：查看响应内容
            self.log(f"登录响应状态码: {resp.status_code}")
            self.log(f"登录响应Content-Type: {resp.headers.get('Content-Type')}")
            self.log(f"登录响应体: {resp.text[:500]}")
            ...
            try:
                ...
                raise Exception(f"登录响应非JSON格式: {resp.text[:200]}")

            if result.get("code") != 1:
                raise Exception(f"登录失败: {result.get('msg', '未知错误')}")

            # 4. 验证merchant Cookie
            merchant_cookie = temp_session.cookies.get("merchant")
            if not merchant_cookie:
                raise Exception("登录失败: merchant Cookie不存在")

            # 将临时session的Cookie合并到主session
    ...
    def submit_order(self, product_url: str, new_price: float) -> dict:
        """提交订单并生成支付二维码 - 秒取卡6步流程（通过快支付）"""
        self.logs = []

        try:
            # 步骤1: 获取Cookie和商品参数
            cookie, params = self._step1_get_cookie_and_params(product_url, new_price)

            # 步骤2: 提交订单创建
            trade_no = self._step2_submit_order(cookie, params)

            # 步骤3: 获取快支付表单
            kuaizhifu_params = self._step3_get_kuaizhifu_form(cookie, trade_no)

            # 步骤4: 提交快支付获取支付宝表单
            alipay_params = self._step4_post_kuaizhifu(kuaizhifu_params)

            # 步骤5: 请求支付宝网关获取302重定向
            location, alipay_cookies = self._step5_post_alipay_gateway(alipay_params)

            # 步骤6: 跟随重定向获取二维码URL
            qr_url = self._step6_get_redirect_and_qr(location, alipay_cookies)

            # 步骤7: 生成二维码
            self.log("步骤7: 正在生成二维码...")
            from app.utils.qr_generator import generate_qr_base64
            qr_base64 = generate_qr_base64(qr_url)
            self.log(f"步骤7: 二维码生成成功，base64长度: {len(qr_base64)}")
    ...
    def _step1_get_cookie_and_params(self, url: str, new_price: float) -> Tuple[str, dict]:
        """步骤1: GET商品详情页，获取Cookie和参数"""
        self.log(f"步骤1: 正在访问商品页面: {url}")
        
        # 使用临时session（避免旧Cookie影响服务器响应）
        temp_session = requests.Session()
        
        # 使用移动端User-Agent（移除Accept-Encoding，让requests自动处理gzip解压）
        headers = {
            "sec-ch-ua": '"Google Chrome";v="147", "Not.A/Brand";v="8", "Chromium";v="147"',
            "sec-ch-ua-mobile": "?0",
            "sec-ch-ua-platform": '"Windows"',
        ...
        resp = temp_session.get(url, headers=headers)
        resp.raise_for_status()
        ...
        # 提取跟踪Cookie - 处理多个同名Cookie的情况
        cookie = temp_session.cookies.get(self.TRACKING_COOKIE)
        if not cookie:
            cookie = resp.cookies.get(self.TRACKING_COOKIE)
        
        if not cookie:
            raise Exception(f"步骤1: 无法获取跟踪Cookie {self.TRACKING_COOKIE}")
        
        self.log(f"步骤1: 获取Cookie和商品参数成功")
        ...
        # 从pay_box提取支付宝pid
        pid = "8"  # 默认支付宝
        pay_boxes = soup.find_all("div", class_="pay_box")
        for box in pay_boxes:
            box_pid = box.get("data-pid", "")
            if box_pid:
                # 查找包含"支付宝"文本的支付框
                span = box.find("span")
                if span and "支付宝" in span.get_text():
                    pid = box_pid
                    break
        ...
        # 设置价格相关参数（根据真实请求）
        params["feePayer"] = "1"
        params["fee_rate"] = "0.06"
        params["min_fee"] = "0.01"
        
        # 计算支付金额
        price_str = params.get("paymoney", str(new_price))
        params["paymoney"] = price_str
        params["danjia"] = params.get("danjia", price_str)
        params["price"] = params.get("price", price_str)
        
        self.log(f"步骤1: 提取参数: goodid={params.get('goodid')}, cateid={params.get('cateid')}, pid={pid}")
        
        return cookie, params

    def _step2_submit_order(self, cookie: str, params: dict) -> str:
        """步骤2: POST创建订单，获取trade_no"""
        self.log("步骤2: 正在提交订单...")
        ...
        order_data["keywords"] = ""
        order_data["goodid"] = params.get("goodid")
        order_data["cateid"] = params.get("cateid")
        order_data["quantity"] = "1"
        order_data["contact"] = contact
        order_data["email"] = ""
        order_data["pwdforsearch1"] = ""
        order_data["pwdforsearch2"] = ""
        order_data["is_contact_limit"] = params.get("is_contact_limit", "default")
        order_data["limit_quantity"] = params.get("limit_quantity", "1")
        order_data["userid"] = params.get("userid")
        order_data["token"] = params.get("token")
        order_data["cardNoLength"] = params.get("cardNoLength", "0")
        order_data["cardPwdLength"] = params.get("cardPwdLength", "0")
        order_data["is_discount"] = params.get("is_discount", "0")
        order_data["coupon_ctype"] = params.get("coupon_ctype", "0")
        order_data["coupon_value"] = params.get("coupon_value", "0")
        order_data["sms_price"] = params.get("sms_price", "0")
        order_data["paymoney"] = params.get("paymoney")
        order_data["danjia"] = params.get("danjia")
        order_data["is_pwdforsearch"] = params.get("is_pwdforsearch", "")
        order_data["is_coupon"] = params.get("is_coupon", "")
        order_data["price"] = params.get("price")
        order_data["kucun"] = params.get("kucun", "0")
        order_data["select_cards"] = params.get("select_cards", "")
        order_data["feePayer"] = params.get("feePayer", "1")
        order_data["fee_rate"] = params.get("fee_rate", "0.06")
        order_data["min_fee"] = params.get("min_fee", "0.01")
        order_data["rate"] = params.get("rate", "100")
        order_data["pid"] = params.get("pid", "8")  # 支付宝
        
        # 请求头
        headers = {
            "host": "www.miaoquka.cn",
            "content-type": "application/x-www-form-urlencoded",
            "cache-control": "max-age=0",
            "sec-ch-ua": '"Google Chrome";v="147", "Not.A/Brand";v="8", "Chromium";v="147"',
            "sec-ch-ua-mobile": "?0",
            "sec-ch-ua-platform": '"Windows"',
            "origin": "https://www.miaoquka.cn",
            "upgrade-insecure-requests": "1",
            "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 18_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.5 Mobile/15E148 Safari/604.1",
            "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
            "sec-fetch-site": "same-origin",
        ...
        if not trade_no_input or not trade_no_input.get("value"):
            raise Exception("步骤2: 未找到trade_no")
        
        trade_no = trade_no_input["value"]
        return trade_no

    def _step3_get_kuaizhifu_form(self, cookie: str, trade_no: str) -> dict:
        """步骤3: GET获取快支付表单"""
        self.log(f"步骤3: 正在获取快支付表单, trade_no={trade_no}")
        
        # 使用真实请求的URL格式（带.html后缀）
        url = f"{self.BASE_URL}/index/pay/payment.html"
        ...
        headers = {
            ...
            "sec-fetch-user": "?1",
            "sec-fetch-dest": "document",
            "referer": f"{self.BASE_URL}/index/pay/payment?trade_no={trade_no}&agree=on",
            "accept-encoding": "gzip, deflate, br, zstd",
            "accept-language": "zh-CN,zh;q=0.9",
            "priority": "u=0, i",
        }
        ...
        resp = self.session.get(
            url,
            params={"trade_no": trade_no, "tip": "0"},
            headers=headers
        )
        resp.raise_for_status()
        ...
        if not form:
            raise Exception("步骤3: 未找到快支付表单(form1)")
        ...
        for input_elem in form.find_all("input", attrs={"type": "hidden"}):
            ...
            value = input_elem.get("value")
            if name and value:
                kuaizhifu_params[name] = value
        
        # 验证必要参数
        required = ["money", "name", "notify_url", "out_trade_no", "pid", "return_url", "sitename", "type", "sign", "sign_type"]
        for key in required:
            if key not in kuaizhifu_params:
                raise Exception(f"步骤3: 快支付表单缺少必要参数: {key}")
        
        self.log(f"步骤3 提取快支付参数成功: {json.dumps(kuaizhifu_params, ensure_ascii=False)}")
        return kuaizhifu_params

    def _step4_post_kuaizhifu(self, kuaizhifu_params: dict) -> dict:
        """步骤4: POST快支付获取支付宝表单"""
        self.log("步骤4: 正在提交快支付...")
        
        # 快支付网关
        kuaizhifu_url = "https://kuaizhifu.cn/submit.php"
        ...
        headers = {
            ...
            "sec-fetch-mode": "navigate",
            "sec-fetch-dest": "document",
            "referer": "https://www.miaoquka.cn/",
            "accept-encoding": "gzip, deflate, br, zstd",
            "accept-language": "zh-CN,zh;q=0.9",
            "priority": "u=0, i",
        }
        
        # URL编码（使用quote_plus，空格编码为+）
        encoded_data = urlencode(kuaizhifu_params, quote_via=quote_plus)
        ...
        if not form:
            raise Exception("步骤4: 未找到支付宝表单(alipaysubmit)")
        ...
        # 验证必要参数
        if "sign" not in alipay_params:
            raise Exception("步骤4: 支付宝表单缺少sign参数")
        
        self.log(f"步骤4 提取支付宝参数成功: {json.dumps({k: v[:50] + '...' if len(v) > 50 else v for k, v in alipay_params.items()}, ensure_ascii=False)}")
        return alipay_params

    def _step5_post_alipay_gateway(self, alipay_params: dict) -> Tuple[str, dict]:
        """步骤5: POST支付宝网关获取302重定向"""
        self.log("步骤5: 正在请求支付宝网关...")
        
        # 支付宝网关URL（不带charset参数）
        ...
        param_order = [
            ...
            "sign_type", "method", "timestamp", "notify_url", "return_url",
            "biz_content", "sign"
        ]
        ...
        if resp.status_code not in (302, 301):
            raise Exception(
                f"步骤5: 预期302重定向，实际HTTP {resp.status_code}\n"
                f"响应摘要: {resp.text[:200]}"
            )
        
        location = resp.headers.get("Location")
        if not location:
            raise Exception("步骤5: 响应头中无Location字段")
        
        alipay_cookies = dict(resp.cookies)
        return location, alipay_cookies

    def _step6_get_redirect_and_qr(self, location: str, alipay_cookies: dict) -> str:
        """步骤6: 跟随重定向获取二维码URL"""
        self.log("步骤6: 正在跟随重定向...")
        ...
        # 获取最终的Location（用于生成二维码）
        if resp.status_code in [301, 302]:
            qr_url = resp.headers.get("Location")
            if not qr_url:
                raise Exception("步骤6: 重定向响应中无Location字段")
        else:
            # 如果不是重定向，使用当前URL
            qr_url = resp.url
    ...
    def get_balance(self) -> dict:
        ...
        try:
            ...
            self.log(f"查询到余额: 可提现={withdrawable}, 不可提现={non_withdrawable}")
