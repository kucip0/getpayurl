...
if resp4.status_code == 200:
    ...
    for i, form in enumerate(forms):
        ...
        for inp in inputs[:3]:
            val = inp.get('value', '')
            print(f"    {inp.get('name')}: {val[:50]}..." if len(val) > 50 else f"    {inp.get('name')}: {val}")
    
    # 查找script中的跳转逻辑
    scripts = soup4.find_all("script")
    print(f"\n找到 {len(scripts)} 个脚本")
    for i, script in enumerate(scripts):
        if script.string and ("location" in script.string or "redirect" in script.string or "submit" in script.string or "href" in script.string):
            print(f"\n脚本 {i} 包含跳转/提交逻辑:")
